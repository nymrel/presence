...
Opening the current LAN console counts as a human arriving. Anything able to reach that URL can produce the present `human.attached` event. The repository states this openly in `docs/COMPLIANCE.md`; it is acceptable only for the current single-operator experimental LAN posture and is not an enterprise identity guarantee.

The planned fix is passkey-bound attach and release. It is **not built**. Until a reviewed WebAuthn implementation satisfies `docs/PASSKEY_THREAT_MODEL.md`, Presence must not claim to prove which human acted.
