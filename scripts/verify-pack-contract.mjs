...
const forbidden = [...files].filter((entry) => (
  entry.startsWith('var/')
  || entry.endsWith('.jsonl')
  || entry.includes('presence-ledger')
  || entry.includes('/tickets/')
));

if (missing.length || forbidden.length) {
  console.error('Presence package contract: FAIL');
  if (missing.length) console.error(`missing required files: ${missing.join(', ')}`);
  if (forbidden.length) console.error(`runtime-state files would be packed: ${forbidden.join(', ')}`);
  process.exit(1);
}
